...
<section class="hero-area">
    @if(isset($cat))
        @if($cat->slider_status == 1)
			@if($cat->slider != null)
			    <div class="row">
                    <div class="item col-12">
                        <div class="itm-dsk">
                                <a href="{{$cat->link}}"><img src="{{asset('assets/images/categories/'.$cat->slider)}}" alt="" style="width:100%; height:400px"></a>
                        </div>
                    </div>
                </div>
			@endif
         ...
         <div class="col-lg-12">
            <ul class="pages">
               ...
               <li>
                  ...
                  <input type="hidden" value="{{$cat->name}}" id="main_cat" data-id="{{$cat->id}}">
               </li>
               @endif
               @if (!empty($subcat))
               <li>
                  <a href="{{route('front.category', [$cat->slug, $subcat->slug])}}">{{ $subcat->name }}</a>
                  <input type="hidden" value="{{$subcat->name}}" id="sub_cat" data-id="{{$subcat->id}}">
               </li>
               @endif
               @if (!empty($childcat))
               <li>
                  <a href="{{route('front.category', [$cat->slug, $subcat->slug, $childcat->slug])}}">{{ $childcat->name }}</a>
                  <input type="hidden" value="{{$childcat->name}}" id="child_cat" data-id="{{$childcat->id}}">
               </li>
               @endif
...
<script>
  ...
  function filter() {
    ...
    $(".brandBox").each(function() {
      if ($(this).is(':checked')) {
        if (filterlink == '') {
          filterlink += '{{route('front.category', [Request::route('category'), Request::route('subcategory'), Request::route('childcategory')])}}' + '?'+'brand[]'+'='+$(this).attr('id');
        } else {
...
<script>
    ...
    sub = $('#sub_cat').val();
    child = $('#child_cat').val();
    mainId = $('#main_cat').attr('data-id');
    subId = $('#sub_cat').attr('data-id');
    childId = $('#child_cat').attr('data-id');

    if (main != null && sub == null && child == null) {
        catTier = 1;
        cat_name = main;
    }
    else if (!main != null && sub != null && child == null) {
        catTier = 2;
        cat_name = sub;
    }
    else if (main != null && sub != null && child != null) {
    ...
    $(document).ready(function() {
        dataLayer.push({
            ...
            'ecommerce': {'detail': [ {"categoryTier":catTier,"content_type":"category","pageType":"category","category":cat_name,"content_category":{main,sub,child},"content_ids":{mainId,subId,childId}} ]}
        })
    })
