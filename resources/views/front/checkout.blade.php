			...
			<div class="row">
                ...
                <div class="col-lg-8">
                    <div class="content-box">
					    <div class="content">
				        <input class="styled-checkbox" id="login-check" type="checkbox" value="1" name="login_check">
				        <label for="login-check">Have an Account?</label>
				            <form id="login-form" action="">
        				        @csrf
        				        <div class="row login-fields d-none">
        				        <div class="col-lg-6">
        						...
        						<div class="col-lg-12 my-2">
        						    <button id="login-btn btn-sm" class="form-control mybtn1">Login</button>
        						</div>
        						</div>
        				    </form>
				    </div>
									...
									<div class="content">
											...
											<div class="row">
												<div class="col-lg-6 {{ $digital == 1 ? 'd-none' : '' }}">
													<select class="form-control" id="shipop" name="shipping" required="">
                                                        ...
                                                        <option value="pickup" >Pickup From ECP Ho</option>
													</select>
												</div>
												...
												<div class="col-lg-6">
													<input class="form-control" type="number" min="11"  name="phone"  id="phone"
												...
												<div class="col-lg-6">
													<textarea class="form-control mb-1" type="text" name="address"  id="address"
													required="" placeholder="{{ $langg->lang155 }}">{{ Auth::guard('web')->check() ? Auth::guard('web')->user()->address : '' }}</textarea>
												</div>
												<div class="col-lg-6">
													<select class="form-control" name="customer_country" required="" id="province">
										...
										<div class="row {{ $digital == 1 ? 'd-none' : '' }}">
											<div class="col-lg-12 mt-3">
													<input class="styled-checkbox" id="ship-diff-address" type="checkbox" value="value1" name="checkother">
										...
										<div class="ship-diff-addres-area d-none">
											...
											<div class="row">
												<div class="col-lg-6">
                                                        <select class="form-control ship_input" name="shipping_city" id="shipping_city">
                                                            @include('includes.cities')
                                                        </select>
												</div>
												{{-- <div class="col-lg-6">
								...
								<div class="content-box">
									<div class="content">
										...
										<div class="order-area">
                                            ...
                                            <input type="hidden" name="ids[]" class="ids" value="{{$product['item']['id']}}">
...
@endif
										...
										<div class="row">
												<div class="col-lg-12 mt-3">
													<div class="bottom-area">
															...
															<button type="submit" id="final-btn" class="mybtn1">Order Now</button>
													</div>
				...
				<div class="col-lg-4">
                    ...
                    <center>
                            <div>
                                <button type="button" id="deleteCoupon" class="btn btn-primary {{((Session::has('already'))?'':'d-none')}}  ">remove</button>
                            </div>
                        </center>
                        ...
                        <div class="shipping-cost">
                            <span><b>Shipping Cost:</b></span><span id="shipping_cost" class="float-right">0</span>
                        </div>
...
$('#shipop').on('change',function(){
	...
	if(val == 'pickup'){
		...
		$('#address').val("Office# 103 First Floor Shafi court's Near Metropole hotel Karachi");
		$('#address').prop('readonly', true);
		$('#province option[value="1"]').prop('selected',true);
		$('#province option[value!="1"]').prop('disabled',true);
		$('#city').html(`<option value="888" selected>Karachi</option>`);
        ...
        $.ajax({
            ...
            success: function (response) {
                var half_cost = response/2;
                $('#shipping_cost').html(half_cost);
                $('#shipping-cost').val(half_cost);
                var ftotal = parseFloat($('#grandtotal').val()) + parseFloat(half_cost);
                ftotal = parseFloat(ftotal);
                $('#final-cost').html('{{ $curr->sign }}'+ftotal)
            }
                    });
        ...
        $('#province option[value!="1"]').prop('disabled',false);
        $('#city').html(`<select class="form-control" name="city" required="" id="city">
														@include('includes.cities')
													</select>`);
                    ...
                    success:function(data){
                        ...
                        else if(data[1]=='001')
                        {
                            ...
                            for (let index = 0; index < data[0].length; index++) {
                                ...
                                toastr.error( data[0][index] +' not Allowed')
                            }
                        }
                        else if(data == 2)
                        {
                        ...
                        else
                        {
                            ...
                            $('#deleteCoupon').removeClass('d-none');
...
// Password Checking
        ...
        $("#open-pass").on( "change", function() {
            if(this.checked){
                $('#login-check').prop('disabled',true);
             ...
             $('#personal-name').prop('required',true);
                ...
                $('#login-check').prop('disabled',false);
        ...
        $("#login-check").on( "change", function() {
            if(this.checked){
            $('#open-pass').prop('disabled',true);
             $('.login-fields').removeClass('d-none');
             $('.login-fields input').prop('required',true);
             $('#login-email').prop('required',true);
             $('#login-password').prop('required',true);
            }
            else{
                $('#open-pass').prop('disabled',false);
             $('.login-fields').addClass('d-none');
             $('.login-fields input').prop('required',false);
             $('#login-email').prop('required',false);
             $('#login-password').prop('required',false);
...
// Step 2 btn DONE
	...
	$('.payment').on('click',function(){
        ...
        if ($("input[name=checkother]").attr('checked',false)) {
            $(document).on('change','#city',function() {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            var ftotal = parseFloat($('#grandtotal').val()) + parseFloat(response);
                            ftotal = parseFloat(ftotal);
                            $('#final-cost').html('{{ $curr->sign }}'+ftotal)
                        }
                    });
        ...
        if($("input[name=checkother]").attr('checked',true))
        {
...
<script>
    $(document).ready(function(e) {
        $.ajax({
            type: "get",
            url: "{{route('checkoutLayer')}}",
            success: function (response) {
                dataLayer.push({ ecommerce: null });
                dataLayer.push({
                    'event': "checkout",
                    'ecommerce': {'detail': [ {"content_type":"product","pageType":"product","content_ids":response.ids,"content_names":response.names,"content_skus":response.skus,"quantities":response.qty,"value":response.prices,"currency":response.currency,"category":response.mains,"sub_category":response.subs,"child_category":response.childs,"brands":response.brands,"sellers":response.sellers} ]}
                    // 'ecommerce': {'detail': [ {"content_type":"product","pageType":"product","content_ids":response.ids,"content_names":response.names,"content_skus":response.skus,"value":"30","currency":"USD","category":response.mains,"sub_category":response.subs,"child_category":response.childs,"brands":response.brands,"sellers":response.sellers} ]}
                })
            }
        });
    })
