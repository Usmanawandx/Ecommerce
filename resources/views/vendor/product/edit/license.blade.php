							...
							<div class="product-description">
							  ...
							  <div class="gocover" style="background: url({{asset('assets/images/'.$gs->admin_loader)}}) no-repeat scroll center center rgba(45, 45, 45, 0.5);"></div>
									...
									<div class="row">
										...
										<div class="col-lg-12">
											<input type="text" class="input-field" placeholder="{{ $langg->lang794 }}" name="sku" readonly required="" value="{{ $data->sku }}">
								  ...
								  @foreach($cats as $cat)
									  <option data-href="{{ route('vendor-subcat-load',$cat->id) }}" value="{{$cat->id}}" {{$cat->id == $data->category_id ? "selected":""}} required>{{$cat->name}}</option>
								  @endforeach
												 </select>
										</div>
									</div>
