	...
	<div class="col-sm-12 remove-padding">
		<a class="item" href="{{ route('front.product', $prod->slug) }}">
			...
			<div class="info">
				...
				<h4 class="price">{{ $attrPrice != 0 ?  $gs->currency_format == 0 ? $curr->sign.$withSelectedAtrributePrice : $withSelectedAtrributePrice.$curr->sign :$prod->showPrice() }} <del><small>{{ $prod->showPreviousPrice() }}</small></del> <small class="text-success">{{$prod->discountPercent()}}</small></h4>
