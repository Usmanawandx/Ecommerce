...
class ChildCategoryController extends Controller
{
   public function __construct()
    ...
    public function import(){
        ...
        return view('admin.childcategory.comissioncsv',compact('cats','sign'));
    }

    public function importComissionCategoryCsv(Request $request){
            $log = "";
            //--- Validation Section
            $rules = [
            ...
            while (($line = fgetcsv($file)) !== FALSE) {
                        if($i != 1)
                        {
                                   ...
                                   $data = Childcategory::where('id', $line[0])->first();
                                   $input["subcategory_id"] = $line[1];
                                   $input["name"] = $line[2];
                                   $input["slug	"] = $line[3];
                                   $input["status"] = $line[4];
                                   $input["comission"] = $line[5];
                                    $data->update($input);
                        }
                        $i++;
                }
                $log .= "<br>Row No: " . $i . " - Product Updated Successfully!<br>";
                $i++;
                fclose($file);
                //--- Redirect Section
    ...
    public function datatables()
    {
         ...
         return Datatables::of($datas)
                            ->editColumn('category',function(Childcategory $data){
                                return'<a href="'.route("front.category",$data->subcategory->category->slug) .'" class="single-category" target="blank">'.$data->subcategory->category->name.'</a>';
                            })
                            ->editColumn('subcategory',function(Childcategory $data){
                                return'<a href="'.route("front.subcat",['slug1'=>$data->subcategory->category->slug,'slug2'=>$data->subcategory->slug]) .'" class="single-category" target="blank">'.$data->subcategory->name.'</a>';
                            })
                            ->editColumn('name',function(Childcategory $data){
                                return'<a href="'.route("front.childcat",['slug1'=>$data->subcategory->category->slug,'slug2'=>$data->subcategory->slug,'slug3'=>$data->slug]) .'" class="single-category" target="blank">'.$data->name.'</a>';
                            })
                            ->addColumn('Comission', function(Childcategory $data) {
                                return $data->subcategory->comission;
                            })
                            ->editColumn('slug',function(Childcategory $data){
                                return'<a href="'.route("front.childcat",['slug1'=>$data->subcategory->category->slug,'slug2'=>$data->subcategory->slug,'slug3'=>$data->slug]) .'" class="single-category" target="blank">'.$data->slug.'</a>';
                            })
                            ...
                            ->rawColumns(['status','category','subcategory','name','slug','attributes','action'])
