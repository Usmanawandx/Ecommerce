    ...
    public function datatables()
    {
         ...
         return Datatables::of($datas)
                            ->editColumn('name',function(Category $data){
                                return'<a href="'.route("front.category",$data->slug) .'" class="single-category" target="blank">'.$data->name.'</a>';
                            })
                            ->editColumn('slug',function(Category $data){
                                return'<a href="'.route("front.category",$data->slug) .'" class="single-category" target="blank">'.$data->slug.'</a>';
                            })
                            ...
                            ->rawColumns(['status','slug','name','attributes','action'])
    ...
    public function update(Request $request, $id)
    {
        ...
        $input = $request->all();
            ...
            if ($file = $request->file('slider'))
            {
                ...
                if($data->slider != null)
                {
                    if (file_exists(public_path().'/assets/images/categories/'.$data->slider)) {
                        unlink(public_path().'/assets/images/categories/'.$data->slider);
                    }
                }
            $input['slider'] = $name;
            }
