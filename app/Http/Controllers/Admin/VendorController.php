	    ...
	    public function datatables()
	    {
	        $datas = User::where('is_vendor','=',2)->orwhere('is_vendor','=',1)->orderBy('id','desc')->get();
	         ...
	         return Datatables::of($datas)
	                            ->addColumn('id', function(User $data) {
	                                return $data->id;
	                            }) 
    ...
    public function update(Request $request, $id)
    {
	    //--- Validation Section
	        $rules = [
                ...
                'shop_slug'   => 'unique:users,shop_slug,'.$id,
                 ];
            $customs = [
                'shop_name.unique' => 'Shop Name "'.$request->shop_name.'" has already been taken. Please choose another name.',
                'shop_slug.unique' => 'Shop Slug "'.$request->shop_slug.'" has already been taken. Please choose another slug.'
            ];
