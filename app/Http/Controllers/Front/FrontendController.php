...
// -------------------------------- HOME PAGE SECTION ----------------------------------------
	...
	public function index(Request $request)
	{
        ...
        $selectable = ['id','user_id','name','slug','features','colors','thumbnail','price','previous_price','attributes','size','size_price','discount_date','photo','thumbnail'];
        $sliders = DB::table('sliders')->get();
        $top_small_banners = DB::table('banners')->where('type','=','TopSmall')->get();
        $ps = DB::table('pagesettings')->find(1);
        $feature_products =  Product::with('user')->where('featured','=',1)->where('status','=',1)->where('stock','<>',0)->select($selectable)->orderBy('id','desc')->take(15)->get()->reject(function($item){
            if($item->user_id != 0){
...
public function vendorLogin()
    ...
    public function extraIndex()
    {
        ...
        $large_banners1 = DB::table('banners')->where('type','=','Large1')->get();
        $large_banners2 = DB::table('banners')->where('type','=','Large2')->get();
        $large_banners3 = DB::table('banners')->where('type','=','Large3')->get();
        $large_banners4 = DB::table('banners')->where('type','=','Large4')->get();
        $large_banners5 = DB::table('banners')->where('type','=','Large5')->get();
        $reviews =  DB::table('reviews')->get();
        $ps = DB::table('pagesettings')->find(1);
        $partners = DB::table('brands')->where('is_feauture',1)->get();
        $selectable = ['id','user_id','name','slug','features','colors','thumbnail','price','previous_price','attributes','size','size_price','discount_date','photo','thumbnail'];
        $discount_products =  Product::with('user')->where('is_discount','=',1)->where('status','=',1)->where('stock','<>',0)->orderBy('id','desc')->take(15)->get()->reject(function($item){
        ...
        $featured_categories = Category::where('is_featured','=',1)->orderBy('id','desc')->get();
        $featured_categories2 = Category::where('is_featured','=',1)->orderBy('id','ASC')->take(6)->get();
        $top_products = Product::with('user')->where('created_at','>','MONTH(CURRENT_DATE - INTERVAL 1 MONTH)')->where('status','=',1)->where('stock','<>',0)->select($selectable)->orderBy('id','desc')->take(15)->get()->reject(function($item){
        ...
        $big_products = Product::with('user')->where('big','=',1)->where('status','=',1)->where('stock','<>',0)->select($selectable)->orderBy('id','desc')->take(15)->get()->reject(function($item){
        ...
        $hot_products =  Product::with('user')->where('hot','=',1)->where('status','=',1)->where('stock','<>',0)->select($selectable)->orderBy('id','desc')->take(15)->get()->reject(function($item){
        ...
        $latest_products =  Product::with('user')->where('latest','=',1)->where('status','=',1)->where('stock','<>',0)->select($selectable)->orderBy('id','desc')->take(15)->get()->reject(function($item){
        ...
        $trending_products =  Product::with('user')->where('trending','=',1)->where('status','=',1)->where('stock','<>',0)->select($selectable)->orderBy('id','desc')->take(15)->get()->reject(function($item){
        ...
        $sale_products =  Product::with('user')->where('sale','=',1)->where('status','=',1)->where('stock','<>',0)->select($selectable)->orderBy('id','desc')->take(15)->get()->reject(function($item){
        ...
        return view('front.extraindex',compact('ps','services','reviews','large_banners1','large_banners2','large_banners3','large_banners4','large_banners5','bottom_small_banners','featured_categories','featured_categories2','top_products','hot_products','latest_products','big_products','trending_products','sale_products','discount_products','partners'));
    }
...
// -------------------------------- PAGE SECTION ----------------------------------------
    ...
    {
        $this->code_image();
        $page =  DB::table('cmspages')->where('page_slug',$slug)->first();
        if(empty($page))
        {
            return response()->view('errors.404')->setStatusCode(404);
        }
        return view('front.cmspage',compact('page'));
    }
// -------------------------------- PAGE SECTION ENDS----------------------------------------
...
// -------------------------------- CONTACT SECTION ----------------------------------------
    ...
    public function paymentMethod()
	{
        ...
        $ps =  DB::table('pagesettings')->where('id','=',1)->first();
		return view('front.paymentmethod',compact('ps'));
	}
    public function returnRefund()
	{
        ...
        $ps =  DB::table('pagesettings')->where('id','=',1)->first();
		return view('front.return&refund',compact('ps'));
	}
...
public function getcartdata()
{
    ...
    $child = $product->childcategory->name;
 ...
 public function catlogExport()
    {
        ...
        $callback = function() use ($prods, $columns)
        {
            ...
            foreach($prods as $prod) {
                ...
                $image_link = 'https://ecpmarket.com/assets/images/products/'.$prod->photo;
                ...
                $name = ucfirst(strtolower($prod->name));
                $description = ucfirst(strtolower($prod->name));
                fputcsv($file, array($prod->id,$name,$prod->sku,$description,$prod->category->name,$stock,$condition,$price,$link,$image_link,$prod->brand->name,$prod->price,$prod->category->name,$prod->subcategory->name,$prod->childcategory->name,$price_range));
            }
            fclose($file);
        };
        ...
        {
            return "you are not authenticate to open this link";
        }
    }
}
