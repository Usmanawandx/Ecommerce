...
class CatalogController extends Controller
{
    ...
    // -------------------------------- CATEGORY SECTION ----------------------------------------
      public function searchByVendor(Request $request){
        $vender = User::where('shop_slug',$request->slug)->firstOrFail();
        $prods = Product::where('vendor_id',$vender->id)->where('status', 1)->where('stock','<>',0)->get();
        $prods = (new Collection(Product::filterProducts($prods)))->paginate(40);
        $data['prods'] = $prods;
        if($request->ajax()) {
        ...
        return view('front.vendorlist', $data,compact('vender'));
      }
      public function searchByBrand(Request $request){
          $brand = Brands::where('slug',$request->slug)->firstOrFail();
          $minprice = $request->min;
            $maxprice = $request->max;
            $category_ids = $request->child_category;
            $sort = $request->sort;
        ...
        //   dd($maxprice);
        $prods = Product::where('brand_id',$brand->id)
                                ->when($category_ids, function ($query, $category_ids) {
                                    return $query->whereIn('childcategory_id', $category_ids);
                                })
                                ...
                                ->when(empty($sort), function ($query, $sort) {
                                    return $query->orderBy('brand_boost', 'DESC');
                                });
        $category_id = $prods->where('status',1)->whereRaw('ifnull(stock,1) > 0')->distinct()->get('childcategory_id');
                                  $prods = $prods->where('status',1)->whereRaw('ifnull(stock,1) > 0')->get();
                                 $relCategories = Childcategory::whereIn('id',$category_id)->get();
        $prods = (new Collection(Product::filterProducts($prods)))->paginate(40);
        $data['prods'] = $prods;
        if($request->ajax()) {
        ...
        return view('front.brand', $data,compact('brand','relCategories'));
      }
    public function category(Request $request, $slug=null, $slug1=null, $slug2=null)
    {
      ...
      $prods = Product::when($cat, function ($query, $cat) {
                                  ...
                                  ->when(empty($sort), function ($query, $sort) {
                                      return $query->orderBy('category_boost', 'DESC');
                                  });
                                ...
                                $brands_id = $prods->where('status', 1)->whereRaw('ifnull(stock,1) > 0')->distinct()->get('brand_id');
                                  $prods = $prods->where('status', 1)->whereRaw('ifnull(stock,1) > 0')->get();
                                 $brands = Brands::whereIn('id', $brands_id)->get();
      $prods = (new Collection(Product::filterProducts($prods)))->paginate(40);
      ...
      return view('front.category', $data,compact('brands'));
    }
...
//        exit();
        ...
        {
            $vendors = Product::where('status','=',1)->where('vendor_id','=',$productt->vendor_id)->take(8)->get();
        }
        else
        {
            $vendors = Product::where('status','=',1)->where('vendor_id','=',null)->take(8)->get();
        }
        $brands = Brands::where("id","=",$productt->brand_id)->first();
        // dd($productt);
        return view('front.product',compact('productt','curr','vendors','brands'));
    }
