...
class CheckoutController extends Controller
{
    ...
    public function Order_invoic(Request $request){
        // $gs= new GeniusMailer();
        ...
        <html lang="en">
          ...
          <body> '.$request->htmll.'   <!-- jQuery first, then Popper.js, then Bootstrap JS -->
...
$headers .= "From: info@ecpmarket.com"."\r\n";

         $res=   mail($to,$subject,$ht,$headers);
        dd($res);
    }
...
try{
        ...
        $order['pay_amount'] = round($request->total + $request->shipping_cost / $curr->value, 2);
        ...
        $order['order_number'] = $this->order_no();
        ...
        // dd($request->shipping_country);
        $order['shipping_province'] = $request->shipping_country;
        ...
        foreach ($cart->items as $key => $item) {
            ...
            $order_items->size_key = $item['size_key'];
            $order_items->size_qty = $item['size_qty'];
            $order_items->size_price = $item['size_price'];
            $order_items->size = $item['size'];
            $order_items->color = $item['color'];
            $order_items->stock = $item['stock'];
            $order_items->item_price = $item['item_price'];
            $order_items->total_price = $item['price'];
            $order_items->license = $item['license'];
            $order_items->dp = $item['dp'];
            $order_items->keys_ = $item['keys'];
            $order_items->keys_ = $item['keys'];
            $order_items->values_ = $item['values'];
            $order_items->product_id = $item['item']->id;
            $vendor = Product::where('id',$item['item']->id)->first(['vendor_id','childcategory_id']);
            $order_items->vendor_id = $vendor['vendor_id'];
            $cc=    Childcategory::find($vendor['childcategory_id']);
            $order_items->admin_commission=$item['price']*($cc->comission/100);
            $order_items->vendor_amount=($item['price']-$order_items->admin_commission);
            $order_items->save();
        }
        ...
        foreach($cart->items as $prod)
        {
            ...
            {
                $product = Product::findOrFail($prod['item']['id']);
                $cc=    Childcategory::find($product->childcategory_id);
                ...
                $vorder->admin_payment=$prod['price']*($cc->comission/100);
                $vorder->vendor_paymment=($prod['price']-$vorder->admin_payment);
...
$validator = Validator::make($input, $rules, $messages);
    ...
    public function findcity()
    {
        ...
        return view('includes.cities',compact('cities'))->render();
    }
    ...
    //
            ...
            $ccat = $pro->childcategory_id;

            $pr = ShippingDetail::where('child_id',$ccat)->where('zone_id',$id)->first();
            if ($pr != "") {
            $cost += $pr->price;
            }
        }
    ...
    public function checkoutLayer()
    {
        ...
        foreach ($cart->items as $key => $item) {
            ...
            array_push($brands,$prod->brand->name);
            array_push($skus,$prod->sku);
            array_push($childs,$prod->childcategory->name);
            array_push($subs,$prod->subcategory->name);
            array_push($mains,$prod->category->name);
            array_push($sellers,$prod->vendor->shop_name);
            array_push($names,$item['item']->name);
