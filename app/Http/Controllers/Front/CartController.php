...
class CartController extends Controller
{
   ...
   public function addcart($id)
    ...
    //    dd($c);
        ...
        $cc=  Session::get('cart');
        $productsNotAllowed=[];
        if (isset($coupon->childcategories_id)||isset($coupon->brands_id)||isset($coupon->users_id)) {

            foreach ($cc->items as $key => $value) {
                // dd($key);
                if ($coupon->childcategories_id!=Product::find($key)->childcategory_id||$coupon->brands_id!=Product::find($key)->brand_id||$coupon->users_id!=Product::find($key)->vendor_id) {

                    // return response()->json(0);
                    Product::find($key)->name;
                    array_push($productsNotAllowed, Product::find($key)->name);
                }
            }
        }
        $productsNotAllowed=[];
        $sku=$coupon->coupons_sku()->get();
        if (count($sku)>0) {

            foreach ($cc->items as $key => $value) {

                // dd($key);
                if ($sku->where('sku',Product::find($key)->sku)->all()) {
        ...
        //     // dd($key);
        //     if ($coupon->childcategories_id!=Product::find($key)->childcategory_id) {
        ...
        // }
        $productsNotAllowed=[];
        if (isset($coupon->min_amount)||isset($coupon->max_amount)) {

                if (!($cc->totalPrice >= $coupon->min_amount  && $cc->totalPrice <= $coupon->max_amount)) {
                    // return 'false';
                    array_push($productsNotAllowed,'This Coupon is');
                }
        }
if (count($productsNotAllowed)>0) {
    $data[0]=$productsNotAllowed;
    $data[1]='001';
    return response()->json($data);
}
        $today = date('Y-m-d');
