...
class ProductController extends Controller
{
    ...
    {
    	 $user = Auth::user();
         $datas =Product::where('vendor_id',$user->id)->where('product_type','normal')->orderBy('id','desc')->get();
    ...
    public function createPhysical()
    {
        ...
        $sign = Currency::where('is_default','=',1)->first();
        return view('vendor.product.create.physical',compact('cats','sign','brand'));
    }
    ...
    public function createDigital()
    {
        ...
        $brand = Brands::where('status','=',1)->get();
        $sign = Currency::where('is_default','=',1)->first();
        return view('vendor.product.create.digital',compact('cats','sign','brand'));
    }
    ...
    public function createLicense()
    {
        ...
        $brand = Brands::where('status','=',1)->get();
        $sign = Currency::where('is_default','=',1)->first();
        return view('vendor.product.create.license',compact('cats','sign','brand'));
    }
...
public function CsvGetColumnIndex($value,$header){
    ...
    public function importSubmit(Request $request)
    {
        ...
        while (($line = fgetcsv($file)) !== FALSE) {
                ...
                if(!Product::where('sku',$line[$sku])->exists() || $line[$sku] == "")
                {
                        $data = new Product;
                        ...
                        if($line[$Image2_Name]!=""){
                            if (file_exists(public_path().'assets/images/galleries/'.$line[$image_batch].'/'.$line[$Image2_Name])) {
                                # code...
                                $inputdata = new Gallery();
                                $input["product_id"] = $data->id;
                                $input['photo'] = $line[$image_batch].'/'.$line[$Image2_Name];
                                ...
                                $this->log.="<br> given image2_name not found according the batch ".$line[$image_batch]." at row ".$i;
                            }
                        }
                        if($this->log==""){
                            $data->fill($input)->save();
                ...
                else
                {
                    ...
                    {
                        $this->log.="<br> You Dont have Permission to Edit this product";
                    }
                }
            $i++;
        }
        ...
        //--- Redirect Section
        $msg = 'Bulk Product File Imported Successfully.<a href="'.route('vendor-prod-index').'">View Product Lists.</a>'. $this->log;
        return response()->json($msg);
    }
    ...
    public function edit($id)
    {
        ...
        if($data->type == 'Digital')
            return view('vendor.product.edit.digital',compact('cats','data','sign','brand'));
        elseif($data->type == 'License')
            return view('vendor.product.edit.license',compact('cats','data','sign','brand'));
        else
            return view('vendor.product.edit.physical',compact('cats','data','sign','brand'));
    }
