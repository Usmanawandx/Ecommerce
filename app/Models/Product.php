...
class Product extends Model
{
    ...
    protected $fillable = ['user_id','category_id','product_type','affiliate_link','sku', 'subcategory_id', 'childcategory_id', 'attributes', 'name', 'photo', 'size','size_qty','size_price','size_prevoius_price','color', 'details','price','previous_price','stock','policy','status', 'views','tags','featured','best','top','hot','latest','big','trending','sale','features','colors','product_condition','ship','meta_tag','meta_description','youtube','type','file','license','license_qty','link','platform','region','licence_type','measure','discount_date','is_discount','whole_sell_qty','whole_sell_discount','catalog_id','slug','brand_id','vendor_id','category_boost','brand_boost','vendor_boost'];
    ...
    else
    {
        ...
        $price =  number_format(($price) * $curr->value);
        if($gs->currency_format == 0){
            return $curr->sign.$price;
        }
        else{
    ...
    public function discountPercent(){
      ...
      if($this->previous_price != 0){
            $percent = (($price / $prePrice) * 100)- 100;
